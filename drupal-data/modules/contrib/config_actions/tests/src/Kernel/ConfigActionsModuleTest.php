...
class ConfigActionsModuleTest extends KernelTestBase {
  ...
  public function testEnable() {
    ...
    $this->assertEquals('myproject_image', $field_storage['field_name'], 'Field storage has correct field_name.');
    $this->assertEquals('node.myproject_image', $field_storage['id'], 'Field storage has correct id.');

    // Test that the field instance config got created.
    $field_instance = $this->getConfig('field.field.node.article.myproject_image');
    $this->assertEquals('myproject_image', $field_instance['field_name'], 'Field instance has correct field base.');
    $this->assertEquals('node.article.myproject_image', $field_instance['id'], 'Field instance has correct id.');
    $this->assertEquals(['field.storage.node.myproject_image', 'node.type.article'], $field_instance['dependencies']['config'],
    ...
    $this->assertEquals('Test short date', $date_config['label']);
